...
func enemy_animation_handler():
	if velocity.x == 0 and not spotted and not dead and not burn and not sliced and not Hit and not on_fire:#if stationary and not spotted player this instant
		sprite.play("Idle")#play idle animation
		Idle=true#i am idling
	if velocity.x != 0 and not spotted and not dead and not burn and not sliced and not Hit and not on_fire:#if moving and not spotted player this instant
		sprite.play("Walk")#play walk anim
		Idle=false#i am not idling
	if spotted and not dead and not burn and not sliced and not Hit:#it stays true only for a second, just needed to play spot anim
		sprite.play("Spot")
...
func _on_spot_timer_timeout():#timer to stop the spot animation
	spotted = false#i have stopped spot anim
	if not Hit:
		speed = default_speed * 2#i run after i spotted the enemy
